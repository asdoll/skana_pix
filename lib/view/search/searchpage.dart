...
class _SearchRecommmendUserPageState extends State<SearchRecommmendUserPage> {
  late ListUserController controller;
  @override
  void initState() {
    super.initState();
    controller = Get.put(ListUserController(userListType: UserListType.recom),
        tag: "search_user");
    controller.reset();
  }
...
class _SearchRecommendPageState extends State<SearchRecommendPage> {
  late HotTagsController hotTagsController;

  @override
  void initState() {
    super.initState();
    hotTagsController = Get.put(HotTagsController(widget.type),
        tag: "hotTags_${widget.type.name}");
    hotTagsController.reset();
  }

  @override
  void dispose() {
    super.dispose();
    hotTagsController.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Obx(
      () => hotTagsController.loadingState.value == LoadingState.loading
          ? progressIndicator(context)
          : CustomScrollView(
              physics: const BouncingScrollPhysics(),
              slivers: [
                          ...
                          ? Padding(
                              ...
                              child: Wrap(
                                ...
                                children: [
                                  if (widget.type == ArtworkType.ILLUST)
                                    for (var f
                                        in hotTagsController.tagExpand.value
                        ...
                        return InkWell(
                          ...
                          child: Card(
                            ...
                            child: Stack(
                              children: <Widget>[
                                PixivImage(
                                  hotTagsController.tags[index].illust.images
                                      .first.squareMedium,
                                  fit: BoxFit.cover,
                                ),
                                Opacity(
